...
class Place:
    ...
    Attributes:
        ...
        timeseries (Series): Time series data for the place.
    ...
    def __init__(self, place_id: str, timeseries: pd.Series, parent: Dataset) -> None:
        ...
        self.parent = parent
        self.indexes = parent.properties.year_ids
        self.columns = parent.properties.sub_season_ids
        self.current_season = timeseries[parent.climatology_end_index : ].to_numpy()

        past_seasons = timeseries[parent.season_shift : parent.climatology_end_index]
        timeseries_reshaped = past_seasons.values.reshape(len(self.indexes), len(self.columns))
        self.seasons_df = pd.DataFrame(timeseries_reshaped, index=self.indexes, 
        ...
        self.seasonal_climatology = self.seasons_df.loc[self.parent.properties.climatology_year_ids]
        self.seasonal_monitoring = self.seasons_df.iloc[:, parent.season_start_index:parent.season_end_index]
        self.seasonal_mon_sel = self.seasonal_monitoring.loc[self.selected_years]
        self.seasonal_mon_clim = self.seasonal_monitoring.loc[self.parent.properties.climatology_year_ids]
        
        # calculate derived dataframes
        self.current_cumsum = np.cumsum(self.current_season)
        self.current_cumsum_mon = np.cumsum(self.current_season_monitoring)
        self.current_index = len(self.current_season_monitoring) - 1

        self.seasonal_cumsum = self.seasonal_monitoring.cumsum(axis=1)
        self.seasonal_current_totals = self.seasonal_cumsum.iloc[:, self.current_index].to_numpy()
        self.seasonal_ensemble = get_ensemble(self.current_season_monitoring, 
                                              self.seasonal_monitoring)
        self.selected_seasons_cumsum = self.seasonal_mon_sel.cumsum(axis=1)
        self.selected_seasons_ensemble = get_ensemble(self.current_season_monitoring, 
                                                      self.seasonal_mon_sel)
        
        self.clim_seasons_cumsum = self.seasonal_mon_clim.cumsum(axis=1)
        self.clim_seasons_ensemble = get_ensemble(self.current_season_monitoring, 
        ...
        self.place_general_stats = pd.Series({
            'Current Season Pctl.': percentiles_from_values(self.seasonal_current_totals, 
                                                            [self.current_cumsum_mon[-1]])[0],
            'Drought Severity Pctls.': percentiles_to_values(self.seasonal_current_totals, 
                                                             (3, 6, 11, 21, 33, 67)).tolist(),
            'Pctls.': self.clim_seasons_pctls.tolist(),
            'Avg.': self.seasonal_climatology.mean().to_list(),
            'Current Season': self.current_season.tolist(),
            'Current Season Accumulation': self.current_cumsum_mon.tolist(),
    ...
    def get_place_stats(self, seasonal_cumsum: pd.DataFrame, seasonal_ensemble: pd.DataFrame):
        ...
        seasonal_general_stats = pd.Series({
            ...
            'C. Dk./LTA': self.current_cumsum_mon[-1]/lta_upto_current_season,
            'Ensemble Med./LTA': ensemble_median[-1]/seasonal_lta[-1],
            'Ensemble Med. Pctl.': percentiles_from_values(seasonal_totals, [ensemble_median[-1]])[0],
            'St. Dev.': standard_dev[-1],
            '33 E. Pctl.': ensemble_pctls[0],
            '67 E. Pctl.': ensemble_pctls[1],
            'E. Prob. Below Normal': ensemble_pctl_probabilities[0],
            'E. Prob. Between Normal': ensemble_pctl_probabilities[1],
            'E. Prob. Above Normal': ensemble_pctl_probabilities[2],
        }, 
        name=self.id)
